...
""",
    ...
    if args.graph_mamba_config.strip():
        gm_explicit = Path(args.graph_mamba_config).expanduser()
        if not gm_explicit.is_absolute():
            for base in (Path.cwd(), *bases):
                cand = (base / gm_explicit).resolve()
                if cand.is_file():
                    gm_cfg = cand
                    break
            else:
                gm_cfg = gm_explicit.resolve()
        else:
            gm_cfg = gm_explicit.resolve()
        # If the resolved config still does not exist (e.g. installed package
        # layout), fall back to DEEPGVS_GRAPH_MAMBA_CONFIG before giving up.
        if not gm_cfg.is_file():
            _gm_default = default_graph_mamba_config_path()
            if _gm_default is not None and _gm_default.is_file():
                print(f"[warn] graph-mamba-config not found at {gm_cfg}; using {_gm_default}")
                gm_cfg = _gm_default
            else:
                raise FileNotFoundError(
                    f"Graph-Mamba config not found: {gm_cfg}. Pass --graph-mamba-config "
                    "/path/to/graph_mamba_infer.json or set DEEPGVS_GRAPH_MAMBA_CONFIG."
                )
    else:
        _gm_default = default_graph_mamba_config_path()
    ...
    with warnings.catch_warnings():
        ...
        if args.features182.strip():
            ...
            print(
                f"[3/3] preprocess (2146->{expected_base_input_dim(model_dir)}) + stacking inference..."
            )
