...
def align_raw_features_to_bundle(
    X_raw: np.ndarray,
    cols: Sequence[str],
    bundle_cols: Sequence[str],
) -> np.ndarray:
    """Reorder an (N, D) raw matrix given in ``cols`` order to ``bundle_cols`` order.

    The MI-selection/PCA inside preprocess.joblib are fitted on the bundle's own
    column order, which can differ from feature_schema.json (e.g. DNA-first vs
    protein-first). Always align before ``pre.transform`` — dim-only checks pass
    silently on mis-ordered matrices and produce wrong predictions.
    """
    cols = list(cols)
    bundle_cols = list(bundle_cols)
    if X_raw.shape[1] != len(cols):
        raise ValueError(f"Raw features {X_raw.shape[1]} != {len(cols)} provided columns")
    if len(cols) != len(bundle_cols) or set(cols) != set(bundle_cols):
        missing = [c for c in bundle_cols if c not in cols]
        extra = [c for c in cols if c not in bundle_cols]
        raise ValueError(
            f"Feature set mismatch vs preprocess.joblib. missing={missing[:5]} extra={extra[:5]}"
        )
    if cols == bundle_cols:
        return X_raw
    idx = [cols.index(c) for c in bundle_cols]
    return np.ascontiguousarray(X_raw[:, idx])
...
def validate_model_bundle(model_dir: Path) -> int:
    ...
    if list(feature_cols) != list(schema_cols):
        print(
            "[warn] preprocess.joblib column order differs from feature_schema.json; "
            "inference will reorder raw features to the bundle order before transform."
        )
    return out_dim
...
def preprocess_raw_features(
    ...
    *,
    ...
    if pre is None or feature_cols is None:
        pre, feature_cols = load_preprocess_bundle(model_dir)
    # X_raw arrives in feature_schema.json order; preprocess.joblib was fit on its
    # own bundle order. Reorder before transform (see align_raw_features_to_bundle).
    src_cols = list(raw_cols) if raw_cols is not None else list(feature_cols)
    aligned = align_raw_features_to_bundle(X_raw, src_cols, list(feature_cols))
    return pre.transform(aligned)
...
def predict_proba(
    ...
    *,
    ...
    ``raw_cols`` is the column order of ``X_raw`` (feature_schema.json order).
    It is aligned to preprocess.joblib's bundle order inside.
    """
    model_dir = Path(model_dir)
    dev = device or torch.device("cpu")
    pre, feature_cols = load_preprocess_bundle(model_dir)
    schema_cols = load_feature_schema_cols(model_dir)
    X182 = preprocess_raw_features(
        X_raw, model_dir, pre, feature_cols, raw_cols=raw_cols or schema_cols
    )
    learners = load_base_learners(model_dir)
...
def predict_labels(
    ...
    *,
    ...
    probas = predict_proba(X_raw, model_dir, device=device, raw_cols=raw_cols)
    p_vf = probas[:, 1] if probas.shape[1] > 1 else probas[:, 0]
    labels = (p_vf >= float(threshold)).astype(np.int64)
    return p_vf, labels
